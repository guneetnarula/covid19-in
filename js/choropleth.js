  ...
  $.fn.choropleth_map = function (data, metadata) {
    return this.each(function () {
      ...
      function createElements() {
        ...
        $("#st_aff")
          .empty()
          .append(metadata[6]["Value"] + "/37");
        $("#dt_aff")
          .empty()
          .append(metadata[5]["Value"] + "/729");
        ...
        $("#ct-modal").click(function () {
          ...
          $(".modal-body")
            .empty()
            .append(
              '<p>If you are familiar with Leaflet and jQuery, check out the <a href="https://github.com/guneetnarula/covid19-in" target="_blank">git repo</a> and contribute</p><p>If you are a journalist or someone carefully tracking news, you can help maintain the data. See the <a href="https://docs.google.com/spreadsheets/d/e/2PACX-1vRlSCAn1nS4h9n9Fp25iuOsH54RfMUjj3xX5CZqjGUqYCVXgwgtJojuqVeqekazs2TkSJ95Jwplo7lL/pubhtml#" target="_blank"">readme sheet here</a>.</p>'
            );
        });
      ...
      function drawMap() {
        ...
        var hybAttrib =
          'Map data © <a href="http://openstreetmap.org" target="_blank">OpenStreetMap</a> contributors & <a href="http://datameet.org" target="_blank">Data{Meet}</a>';
        var hyb = new L.TileLayer(hybUrl, {
          minZoom: 4,
          maxZoom: 12,
          attribution: hybAttrib,
          opacity: 1,
        }).addTo(map);

        //---------------ADD DISTRICT BOUNDARIES

        gjLayerDist = L.geoJson(geodist, {
          style: styledist,
        ...
        for (var i = 0; i < data.length; i++) {
          if (data[i]["District"] == feature.properties["dtname"]) {
            return (
              ...
              "</h4><hr><p>Confirmed Cases: <b>" +
              data[i]["Confirmed Cases"] +
              "</b> out of " +
              counter("State", feature) +
              " in the state</p><p>Discharged/Recovered: " +
              data[i]["Discharged"] +
              "</p><p>Deaths: " +
              data[i]["Deaths"] +
              "</p><p>Active Cases: " +
      ...
      function counter(level, feature) {
        ...
        for (var i = 0; i < data.length; i++) {
          if (
            data[i][level] == feature.properties[property] &&
            data[i]["State"] == feature.properties["stname"]
          ) {
      ...
      function onEachDist(feature, layer) {
        ...
        layer.bindTooltip(
          feature.properties["dtname"] + ", " + feature.properties["stname"],
          {
            direction: "auto",
...
function allCounts(data, level, name) {
  var totals = [0, 0, 0, 0];
  for (var i = 0; i < data.length; i++) {
...
//DISTRICTS MODAL
function districtData(sortBy = "Confirmed Cases", sortDir = "DESC") {
  var data = JSON.parse(localStorage.getItem("data"));
  var arrow = sortDir == "DESC" ? "↓" : "↑";

  if (sortDir == "DESC") data.sort((a, b) => compare(a[sortBy], b[sortBy]));
  else data.sort((a, b) => compareR(a[sortBy], b[sortBy]));

  var districtHTML = `<table class='dt-data'>
  <tbody>
  	<tr>
  		<th data-key='District'>${
        "District " + (sortBy == "District" ? arrow : "")
      }</th>
	  <th data-key='State'>${"State " + (sortBy == "State" ? arrow : "")}</th>
	  <th data-key='Confirmed Cases'>${
      "Confirmed Cases " + (sortBy == "Confirmed Cases" ? arrow : "")
    }</th>
	  <th data-key='Discharged'>${
      "Discharged/ Recovered " + (sortBy == "Discharged" ? arrow : "")
    }</th>
	  <th data-key='Deaths'>${"Deaths " + (sortBy == "Deaths" ? arrow : "")}</th>
	  <th data-key='Active'>${
      "Active Cases " + (sortBy == "Active" ? arrow : "")
    }</th>
	  <th data-key='Notes'>${"Notes " + (sortBy == "Notes" ? arrow : "")}</th>
	</tr>`;
...
function stateData(sortBy = "cc", sortDir = "DESC") {
  var s_totals = [];
  var flag = 0;
  var data = JSON.parse(localStorage.getItem("data"));
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (flag == 0) {
      var totals = allCounts(data, "State", data[i]["State"]);
      s_totals.push({
        name: data[i]["State"],
        cc: totals[0],
        di: totals[1],
  ...
  if (sortDir == "DESC") s_totals.sort((a, b) => compare(a[sortBy], b[sortBy]));
  else s_totals.sort((a, b) => compareR(a[sortBy], b[sortBy]));

  //SORT BY CONFIRMED CASES
  //console.log(s_totals);
  var stateHTML = `<table class='st-data'>
	<tbody>
		<tr>
			<th data-key='name'>${"State " + (sortBy == "name" ? arrow : "")}</th>
			<th data-key='cc'>${"Confirmed Cases " + (sortBy == "cc" ? arrow : "")}</th>
			<th data-key='di'>${
        "Discharged/ Recovered " + (sortBy == "di" ? arrow : "")
      }</th>
			<th data-key='de'>${"Deaths " + (sortBy == "de" ? arrow : "")}</th>
			<th data-key='ac'>${"Active Cases " + (sortBy == "ac" ? arrow : "")}</th>
		</tr>`;
...
function sort_StateData(sortBy = "", dir) {
  $(".st-data").html(stateData(sortBy, dir));
}

function sort_DtData(sortBy = "", dir) {
  $(".dt-data").html(districtData(sortBy, dir));
}

jQuery(document).ready(function () {
  Tabletop.init({
    key: "1AL1cj_33m3D7JkT-_wPB7LPJAqIfV2Y5XVMui7nczy4",
    callback: getdata,
    simpleSheet: false,
  });
  ...
  jQuery(".modal-body").on("click", ".st-data th", function (event) {
    var dir = $(this).text().indexOf("↓") > -1 ? "ASC" : "DESC";
    sort_StateData($(this).data("key"), dir);
  });

  jQuery(".modal-body").on("click", ".dt-data th", function (event) {
    var dir = $(this).text().indexOf("↓") > -1 ? "ASC" : "DESC";
    sort_DtData($(this).data("key"), dir);
  });
});
